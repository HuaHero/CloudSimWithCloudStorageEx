...
public class StorageDatacenterCharacteristics_Origin{
	 ...
	 * @post $none
	 */
	public StorageDatacenterCharacteristics_Origin(
			String architecture,
			String os,
			String vmm,
			List<? extends StorageNode> nodeList,
			double timeZone,
			double costPerSec,
			double costPerMem,
     ...
     * @post $none
     */
    public StorageNode getNodeWithFreePe() {
        return StorageNodeList.getNodeWithFreePe(getNodeList());
    }
     ...
     * @post $none
     */
    public StorageNode getNodeWithFreePe(int peNumber) {
        return StorageNodeList.getNodeWithFreePe(getNodeList(), peNumber);
    }
	...
	public int getMipsOfOnePe() {
        ...
        return PeList.getMips((List<Pe>) getNodeList().get(0).getPeList(), 0);
    }
	...
	public int getMipsOfOnePe(int id, int peId) {
        ...
        return PeList.getMips((List<Pe>) StorageNodeList.getById(getNodeList(), id).getPeList(), peId);
    }
    ...
    public int getMips() {
        ...
        switch (getAllocationPolicy()) {
            ...
            case DatacenterCharacteristics.OTHER_POLICY_SAME_RATING:
                mips = getMipsOfOnePe() * StorageNodeList.getPesNumber(getNodeList());
                break;
            ...
            case DatacenterCharacteristics.OTHER_POLICY_DIFFERENT_RATING:
            	for (StorageNode node : getNodeList()) {
            		int peSize = node.getPeList().size();
            		for(int j=0;j<peSize;j++){
            			mips += node.getPeList().get(j).getMips();
            		}
            	}
                break;
    ...
    public int getPesNumber() {
        return StorageNodeList.getPesNumber(getNodeList());
    }
    ...
    public int getFreePesNumber() {
        return StorageNodeList.getFreePesNumber(getNodeList());
    }
   ...
   /* public int getBusyPesNumber() {
        return HostList.getBusyPesNumber(getNodeList());
    }*/
    ...
    public boolean setPeStatus(int status, int hostId, int peId) {
        return StorageNodeList.setPeStatus(getNodeList(), status, hostId, peId);
    }
     ...
     */
    public int getFailedNodesNumber() {
        int failedNodesNumber = 0;
        for (StorageNode node : getNodeList()) {
            if (node.isFailed()) {
            	failedNodesNumber++;
            }
        }
        return failedNodesNumber;
    }
	 ...
	 */
	protected <T extends StorageNode> void setNodeList(List<T> nodeList) {
